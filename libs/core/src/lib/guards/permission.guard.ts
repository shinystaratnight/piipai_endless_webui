...
import { UserService, NavigationService, CheckPermissionService } from '../services';
    ...
    return this.userServise
      ...
      .pipe(
        mergeMap((user: User) => {
          ...
          const requests = [of(user), this.navigationService.getPages(user.currentRole, companyId)];
